...
var XClosed = false;
var fromPopUp = false;
var variableMQuery = "(-webkit-nintendo-switch-device-mode: -webkit-nintendo-switch-device-";
...
var newsURL = "/news/";
var newsImageURL = "/news/images/";
// Load the news articles
// loadNews("first");  commented out for high bandwidth usage, now runs in the populateData func
...
function populateData(event){
	...
	switch(event){
		...
		case 'three': // Feedback tab
			...
			$(".title").html("Feedback");
			break;
		case 'four': // Useful Links tab
			$(".title").html("Useful Links");
			selected = "outer-links";
			break;
		case 'five': // BrowseDNS tab
			$(".title").html("BrowseDNS.net");
			selected = "outer-browse";
			break;
	...
	$.ajax({
		...
		success: function(data) {
			if(data.error !== true) { // If there was no error
				...
				news_text = $('<div class="ignore">'+news_text+'</div>');
				$(news_text.find("nl").get().reverse()).each(function() {
					$(this).replaceWith("<br><br>"+$(this).html());
				});
				news_text = news_text[0].outerHTML;
				...
				if($(news_text).find("button").length) {
					...
					if($b.attr("url")) {
						var url = $b.attr("url");
						var code = '<input type="submit" id="news-button" value="'+i+'" tabindex="-1" up="back-button" left="outer-news" down="like">';
	...
	$.ajax({
		...
		success: function(data) {
			if(data.error == true && data.allowed == false) { // If the user has not yet allowed their IP address
				if(confirm("In order to continue, we will need to store your IP address on our servers. This IP will be used to prevent multiple votes on the same article. Do you accept?")) {
...
function survey() {
	window.location.href = "https://docs.google.com/forms/d/e/1FAIpQLScE9QjXEl0vdOcAl7ixjyuNjgffYdp08ism16VlFPfoh_hKGA/viewform";
}
...
function linkScroll() {
	...
	else if(sID == "discord-link") {
		$("#content").animate({
			scrollTop: $("#content").scrollTop() - $("#content").offset().top + $("#content #discord-link").first().offset().top 
		}, t);
	}
	else if(sID == "change-org") {
		$("#content").animate({
			scrollTop: 0
		}, t);
	}
...
window.onpopstate = function(event) {
	...
	var parsed = event.state
	switch(parsed.page) { // Check the current page
...
function consoleMode() {
	if(matchMedia(variableMQuery+"handheld)").matches) {
		return "handheld";
	}
	else if(matchMedia(variableMQuery+"console)").matches) {
		return "docked";
	}
}
...
function resetPopup() {
	...
	$("#overlay .new, #overlay .new-request, .code-text, #overlay #online-status, #overlay h3[id='header'], #overlay .became-friends, #overlay .text, .date-text, .sent-by, .blocked-method, .date-blocked-text, #overlay #pinprompter").remove();
}
