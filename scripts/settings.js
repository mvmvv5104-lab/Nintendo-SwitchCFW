...
function settingsTab(tab) {
	...
	switch(tab) {
		case "about": // About tab
			settingsPageContent = `<h2>About SwitchBru Dashboard</h2>
				This website is provided free of charge. It can be accessed on a computer at <strong>dns.switchbru.com</strong> and is hosted on a separate server from <a href="https://switchbru.com/dns/" tabindex="-1" id="google-dns" left="outer-about" down="patreon">Switchbru DNS</a>.
				
				<br><br>We are hosting this service as we believe that those of us that purchased an Internet-capable Switch should have the right to browse the web! We hope that one day Nintendo adds an easily accessible web browser to the console.
				<br><br>
                If you choose to make an IP-based user to store options and bookmarks, or a SwitchBru account to store friends, themes, and messages, this information is collected and saved for later use. We do not share any personal data stored with us with any third parties.
				...
				<br><hr>
				<small><b>v4.4.4</b> &ndash; created by vgmoose, designed by pwsincd, with news, user features and messaging added by <a href="https://www.patreon.com/ep8script" id="patreon" tabindex="-1" left="outer-about" up="google-dns">Ep8Script</small><span class="select-next" selectnext="google-dns"></span>`
...
function checkVersion() {
	...
	var SUA = /Mozilla\/5\.0 \(Nintendo Switch; .*\) AppleWebKit\/.* \(KHTML, like Gecko\) NF\/.* NintendoBrowser\/(.*)/
	if(UA.match(SUA)) { // If the user agent is correct
		var match = SUA.exec(UA);
		switch(match[1]) { // Check the version based on known strings
			case "5.1.0.11682":
				systemVersion = "1.0.0";
				break;
			case "5.1.0.13341":
...
function checkUpdate() {
	if(systemVersion == "Unknown") {
		...
		// Check if the system version is 4.0.0 or below (technically v4.1.0)
		if(v <= 4) {
			alert("You are able to run homebrew on "+systemVersion+" through the web browser exploit.");
		}
		else {
			alert("You can use homebrew on "+systemVersion+" by using the Fusée Gelée exploit.");
		}
	}
}
