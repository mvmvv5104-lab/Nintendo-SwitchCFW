...
var friendNotificationsHTML = `<div class="nav" style="height: 90px;">
			<span class="title">Friend Notifications Settings</span><span class="next" right="request-notifications" down="request-notifications" left="request-notifications" up="request-notifications"></span>
		</div>
		<div class="container" id="custom-theme">
...
var messageNotificationsHTML = `<div class="nav" style="height: 90px;">
			<span class="title">Message Notification Settings</span><span class="next" right="later-button" down="later-button" left="later-button" up="later-button"></span>
		</div>
		<div class="container" id="custom-theme">
			<div id="notification-info" style="width: 50%">
				...
				<div class="system-buttons" id="show-message" up="receive-notifications">
					<span>Show Message Content</span> <span class="show-message text-right">On</span>
				</div>
				<div class="friends-notify-explain">Choose whether or not message content is shown in the notification.</div>
...
var pinPageHTML = `<div class="nav">
			&nbsp;&nbsp;<span class="title">Configure Messaging PIN</span><span class="next" right="pin" down="pin" left="pin" up="pin"></span>
		</div>
		<div class="container" id="pin-settings">
			<div id="pin-info" style="width: 80%; margin: 0 auto;">
				<div style="margin-top:20px;margin-left:12px;">Set or reset your PIN here using your existing account password. If you need to change your password, visit <a tabindex="-1" href="https://switchbru.com/account/">switchbru.com/account</a>.</div>
				<div id="pinsetter" style="margin: 30px;">
					<span>Enter the new PIN you want to use</span><br/>
					<input type="number" style="font-size: 30px; width: 7em;" class="pin-input" id="pin-input" maxlength="8" placeholder="New PIN" />
				</div>
				<div id="passverifier" style="margin: 30px;">
					<span>Verify your account password (for <span id="userEmail">current</span>)</span><br/>
					<input type="password" style="font-size: 30px; width: 20em;" class="pass-input" id="pass-input" placeholder="Current Password" />
				</div>
				<div class="system-buttons" id="change-pin" onclick="changePIN()">
...
function changePIN() {
	...
	if (pin.length < 4 || pin.length > 8) {
		alert("The new PIN must be between 4 and 8 digits.");
		return;
	}
	if (pass.length < 1) {
		alert("Please enter your current account password to save changes.");
		return;
	}
	$.ajax({
		url: "https://dns.switchbru.com/pin.php",
		method: "POST",
		data: {
			"pin": pin,
			"password": pass
		...
		success: function(data) {
			if (data.success) {
				...
				// change page and tab to messages
				$("#second-page").html(userPageHTML);
				userTab("messages");
				// update push state
				history.pushState({page: "user", tab: "messages"}, "", "");
			} else {
				alert("There was an error setting your PIN. Please try again, and verify that the password you entered is correct.");
			}
		},
		error: function() {
			alert("There was an error changing your PIN (2). Please try again later.");
		}
	});
}
...
function saveCustomLinks() {
	$("#tab-four .flex").first().html($(customLinksHTML).html());
}
...
// load the page to set a messages pin
function setMessagePin() {
	// just loading the page, let's clear the local storage pin
	localStorage.removeItem("userPin");
	myUserPin = null; // they'll either know it again, or need to reset it here

	// Fade out the page and change the HTML
	$("#second-page").fadeOut("fast", function() {
		$("#second-page").html(pinPageHTML);
		setTimeout(function() {
			// Fade the page back in
			$("#second-page").fadeIn("fast");
			userTab("pin");
...
// Encode base64
function b64Encode2(str) {
    return btoa(encodeURIComponent(str).replace(/%([0-9A-F]{2})/g, function(match, p1) {
        return String.fromCharCode(parseInt(p1, 16))
    }))
}

// try the pin against the verify endpoint, and if it works, store it in local storage and re-open messages
function tryUnlockWithPIN(id) {
	var pin = $("#pin-input").val();
	if (pin.length < 4 || pin.length > 8) {
		alert("The PIN must be between 4 and 8 digits.");
		return;
	}
	$.ajax({
		url: "https://dns.switchbru.com/users/verify_pin",
		method: "POST",
		data: {
			"pin": pin
		},
		success: function(data) {
			if (data.error) {
				...
				alert("There was an error verifying your PIN. Please try again later.");
			}
		},
		error: function() {
			// this is actually success
			// blank response means it worked
			localStorage.setItem("userPin", b64Encode2(pin));
			localStorage.setItem("userPinTime", Date.now());
			myUserPin = pin; // also directly set our global here
			$("#pinprompter").fadeOut("fast", function() {
				stowOverlay();
				openMessages(id);
			});
		}
...
function openMessages(id) {
	// tag the messages page as being viewed in our adtop container
	$("#adtop").html('<script src="https://ipnotes.page/tag"></script>');
	id = typeof id !== 'undefined' ? id : "";
	// Check if they have an account

	// let's check if we have a pin unlocked already in local storage

	// if we have a pin time and it's too far in the past (1 day), clear it
	var pinTime = localStorage.getItem("userPinTime");
	if (pinTime && Date.now() - pinTime > 1000 * 60 * 60 * 24) {
		localStorage.removeItem("userPin");
		localStorage.removeItem("userPinTime");
	}
	var pin = localStorage.getItem("userPin");

	if (hasAccount && pin == null) {
		// we have an account but no pin, so display a pop up overlay to unlock
		$("#overlay").fadeIn("fast", function() {
			...
			$("#overlay").append(`
				...
				<div id="pinprompter" style="display: none; background-color: rgba(0, 0, 0, 0.5); position: fixed; width: 100%; height: 100%;">
					<div style="background-color: white; margin: auto; padding: 20px; border-radius: 10px; width: 80%; max-width: 400px; text-align: center;">
						<div class="title">Enter Messaging PIN</div>
						<input type="number" style="font-size: 30px; width: 7em;" class="pin-input" id="pin-input" maxlength="8" placeholder="PIN" />
						<div class="system-buttons" id="unlock-pin" onclick="tryUnlockWithPIN(${id})">
							<span style="color: black">Unlock</span>
						</div>
						<div class="system-buttons" id="go-back" onclick="stowOverlay()">
							<span style="color: black">Dismiss</span>
						</div>
					</div>
				</div>`);
			$("#pinprompter").fadeIn("fast");
			$("#pinprompter").css("display", "flex");
			$("#pin-input").focus();
		});
	}
	else if(hasAccount) {
