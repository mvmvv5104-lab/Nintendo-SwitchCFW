...
function Nes() {
  ...
  // SwitchWebCFW performance options. These can be changed from player.html before bootup/run.
  // Default keeps normal NES timing unless player.html sets a lighter value.
  this.switchWebCyclesPerFrame = ((typeof window !== 'undefined' && window.SWITCHWEB_NES_CYCLES) ? window.SWITCHWEB_NES_CYCLES : ((341 * 262 / 3) | 0));
  this.switchWebAudioOff = (typeof window !== 'undefined' && window.SWITCHWEB_NES_AUDIO_OFF !== undefined) ? window.SWITCHWEB_NES_AUDIO_OFF : true;
...
Object.assign(Nes.prototype, {
  ...
  setAudio: function(audio) {
    // SwitchWebCFW: keep audio fully disabled by default for better performance.
    if(this.switchWebAudioOff === true) {
      this.audioEnabled = false;
      return;
    }
  ...
  run: function() {
    ...
    // SwitchWebCFW: allow player.html to lower cycles for slow browsers.
    // Normal NES is about (341 * 262 / 3) CPU cycles per frame.
    var cycles = this.switchWebCyclesPerFrame || ((341 * 262 / 3) | 0);
